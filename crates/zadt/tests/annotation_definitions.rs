...
use zadt::{
    ...
    AnnotationDefinitionPropertiesVersion, Client, EntityTag, Logon, ObjectVersion, Operation,
    Ready, ReqwestTransport,
};

const DISCOVERY_XML: &str = include_str!("fixtures/discovery.xml");
const CORE_DISCOVERY_XML: &str = include_str!("fixtures/core-discovery.xml");
const ANNOTATION_DEFINITION_XML: &str = include_str!("fixtures/annotation-definition-ui.xml");
const ANNOTATION_DEFINITION_MEDIA_TYPE: &str = "application/vnd.sap.adt.ddic.ddla.v1+xml";
...
const SOURCE: &str = "@Scope: [#ENTITY]\ndefine annotation UI.hidden : Boolean;\n";
...
#[tokio::test]
async fn annotation_definition_properties_advertise_the_primary_source() {
    ...
    let properties = server
        .mock_async(|when, then| {
            when.method(GET)
                .path("/sap/bc/adt/ddic/ddla/sources/ui")
                .query_param("version", "active")
                .header("accept", ANNOTATION_DEFINITION_MEDIA_TYPE)
                .header("cache-control", "no-cache");
            then.status(200)
                .header(
                    "content-type",
                    "application/vnd.sap.adt.ddic.ddla.v1+xml; charset=utf-8",
                )
                .header("etag", "annotation-definition-etag")
                .body(ANNOTATION_DEFINITION_XML);
        })
        .await;
    let source = server
        .mock_async(|when, then| {
            when.method(GET)
                .path("/sap/bc/adt/ddic/ddla/sources/ui/source/main")
    ...
    assert_eq!(
        ...
        Some("annotation-definition-etag")
    );
...
#[tokio::test]
async fn annotation_definition_creation_posts_only_the_sparse_properties_payload() {
    let server = MockServer::start_async().await;
    let logon = mock_logon(&server).await;
    let discovery = mock_discovery(&server).await;
    let _core_discovery = mock_core_discovery(&server).await;
